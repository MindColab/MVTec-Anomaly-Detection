...
def get_patch_mask(image, mask, boder_size=5):
    """
    ...
    non_zero_indices = cv2.findNonZero(mask)
...
def run():
    ...
    #Output csv
    csv_out = list()
    out_csv_path = os.path.dirname(args['output_path'])+"/train-patches.csv"
    ...
    for folder in inference_folders:
        ...
        out_image_path = f"{OUTPUT_DIR}/good/"
        ...
        for img in read_images_name(images_list_path):
            ...
            if os.path.exists(mask_path):
                ...
                image, mask = get_patch_mask(image, mask)
                cropped_image, cropped_mask = add_sqr_borders(image, mask)

                '''
                iris_mask = get_iris_mask(mask)
                print("iris_mask_size:",iris_mask.shape)
                image_masked = apply_mask_to_image(image,iris_mask)
                ruptura_mask = iris_mask #(mask,np.array(label_dict['ruptura']))
                cropped_image = crop_window(image_masked,iris_mask,640)
                print("cropped_image_size:",cropped_image.shape)
                cropped_mask = crop_window(ruptura_mask,iris_mask,640)
                '''
                ...
                if CHECK_MASK_EXISTS == False or not im_pil.getbbox() == None:
                    ...
                    mask_out_path = f"{out_mask_path}{imageid}_mask_{prefix_image}_{image_basename}{mask_ext.lower()}"
                    print("out1:",image_out_path)
                    #print("out2:",mask_out_path)
                    cv2.imwrite(image_out_path, cropped_image)

                    # Normaliza los valores de la máscara del parche al rango de 0 a 255
